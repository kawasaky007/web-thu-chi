...
function MonthControls({ month }: { month: string }) {
  return <div className="flex items-center justify-between gap-2 rounded-2xl border border-forest/10 bg-paper-raised/65 p-2"><Link aria-label="Tháng trước" className="relative grid size-11 place-items-center rounded-xl text-forest transition hover:bg-mist" href={`/budgets?month=${shiftMonth(month, -1)}`}><ChevronLeft aria-hidden="true" className="size-5" /><LinkPendingOverlay /></Link><MonthPicker ariaLabel="Chọn tháng ngân sách" hrefForMonth={(value) => `/budgets?month=${value}`} month={month} /><Link aria-label="Tháng sau" className="relative grid size-11 place-items-center rounded-xl text-forest transition hover:bg-mist" href={`/budgets?month=${shiftMonth(month, 1)}`}><ChevronRight aria-hidden="true" className="size-5" /><LinkPendingOverlay /></Link></div>;
}
